...
const db = mysql.createConnection(process.env.DBURI)
db.connect((err) => {
  if (err) throw err;
  ...
  bot.getChatMember(msg.chat.id, msg.from.id)
    .then((member) => {
      if (member.status === "member") {
        ...
        const saveYoutube = mysql.createConnection(process.env.DBURI)
        saveYoutube.connect()
  ...
  bot.getChatMember(msg.chat.id, msg.from.id)
    .then((member) => {
      if (member.status === "member") {
        ...
        const saveSite = mysql.createConnection(process.env.DBURI)
        saveSite.connect()
  ...
  bot.getChatMember(msg.chat.id, msg.from.id)
    .then((member) => {
      if (member.status === "member") {
        ...
        const removeSite = mysql.createConnection(process.env.DBURI)
        removeSite.connect()
  ...
  bot.getChatMember(msg.chat.id, msg.from.id)
    .then((member) => {
      if (member.status === "member") {
        ...
        const removeYoutube = mysql.createConnection(process.env.DBURI)
